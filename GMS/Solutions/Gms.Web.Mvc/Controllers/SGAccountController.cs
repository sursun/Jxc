...
namespace Gms.Web.Mvc.Controllers
{
    ...
    public class SGAccountController : BaseController
    {
        ...
        public virtual ActionResult Login(LogOnViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (authenticationService.LogOn(model.UserName, model.Password, model.RememberMe))
                {
                    ...
                    {
                        var result = ReginfoRepository.Common.ValidRegInfo(CurrentRegInfo);
                        if (result.IsValid)
                        {
                            bReg = true;
                        }
                    ...
                    {
                        //if (IsSystem())
                        if (Roles.IsUserInRole(model.UserName,RoleConstants.SYSTEM))
                        {
                        ...
                        {
                            ModelState.AddModelError("", "产品未注册");
                            return LogOff();
                        }
                    }
        ...
        //[HttpPost]
        public virtual ActionResult AndroidLogin(string Name,string PassWord)
        {
            if (Name == "houck" && PassWord == "123")
                return JsonSuccess(new {Name = "hck",PassWord="456"});
            else
            {
